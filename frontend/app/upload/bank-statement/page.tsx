...
import { ApiError, previewUpload, uploadBankStatement } from "@/lib/api-client";
...
// See the note on the invoice upload page: both upload routes are listed so
// the nav highlights the upload step you are on rather than "Home".
const navItems = [
...
export default function BankStatementUploadPage() {
  ...
  async function submitUpload(fileToSubmit: File, sourceMappingId: string) {
    setStage("submitting");
    setError(null);
    try {
      const result = await uploadBankStatement(fileToSubmit, sourceMappingId);
      setBatch(result);
      setStage("done");
    } catch (err) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 px-6 py-12">
      <header className="flex flex-col gap-4">
        ...
        <h1 className="text-3xl font-extrabold tracking-tight">Bank statement</h1>
        <PillNav items={navItems} activeHref="/upload/bank-statement" />
      </header>

      <Card className="flex flex-col gap-4">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Choose a CSV file</h2>
          <p className="text-sm text-text-muted">
            Bank statements are CSV only and always go through the column-mapping step.
          </p>
        </div>
        <input
          ref={fileInputRef}
      ...
      {stage === "done" && batch && (
        <Card className="flex flex-col gap-3">
          ...
          {batch.error_summary && <p className="text-sm text-danger">{batch.error_summary}</p>}
