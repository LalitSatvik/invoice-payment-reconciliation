...
# The committed corpus that `tests/matching/test_scenarios.py` reads as its
# ground truth. It is generated by this module's generator at DEFAULT_SEED.
COMMITTED_DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "synthetic"

GENERATED_FILES = ("invoices.csv", "bank_statement.csv", "ground_truth.json")
...
def test_committed_seed_matches_the_generator_default():
    """The committed ground truth records its own seed; the drift check below
    regenerates at DEFAULT_SEED, so the two values must agree.
    """
    payload = json.loads(
        (COMMITTED_DATA_DIR / "ground_truth.json").read_text(encoding="utf-8")
    )
    assert payload["seed"] == DEFAULT_SEED


@pytest.mark.parametrize("name", GENERATED_FILES)
def test_committed_fixtures_are_byte_identical_to_a_fresh_generation(name, tmp_path):
    """Guards the project's central regression corpus against silent drift.

    ``backend/data/synthetic/*`` is committed to the repo and read as fixed
    ground truth by ``tests/matching/test_scenarios.py``. Nothing otherwise
    ties those committed bytes to the generator that produced them, so an
    edit to ``scenarios.py`` without a regeneration would leave the guardrail
    suite silently asserting against a stale corpus. Regenerating at the same
    seed and diffing byte-for-byte closes that gap: if this fails, re-run
    ``python -m app.synthetic.scenarios`` and commit the result.
    """
    dataset = build_dataset(DEFAULT_SEED)
    write_invoice_csv(dataset.invoices, tmp_path / "invoices.csv")
    write_payment_csv(dataset.payments, tmp_path / "bank_statement.csv")
    write_ground_truth(dataset, tmp_path / "ground_truth.json")

    committed = (COMMITTED_DATA_DIR / name).read_bytes()
    regenerated = (tmp_path / name).read_bytes()
    assert committed == regenerated, (
        f"committed backend/data/synthetic/{name} is stale -- regenerate it with "
        f"`python -m app.synthetic.scenarios` and commit the result"
    )
